        ...
        if len(clients_avatar) > 0:
            ...
            clients_container = ft.Container(
                content=ft.Row(
                    [
                        ft.Icon(
                            name=ft.Icons.ERROR_OUTLINE, color=ft.Colorss.AMBER_500, size=32 # Changed to a specific Amber shade
                        )
                    ],
                    spacing=10,
                ),
    ...
    async def periodic_status_check(self):
        ...
        while self.running:
            ...
            if status["state"] == "Online":
                self.status_indicator.bgcolor = ft.Colorss.GREEN_500 # Theme-friendly green
            else:
                self.status_indicator.bgcolor = ft.Colorss.RED_500   # Theme-friendly red
